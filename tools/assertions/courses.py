...
def assert_create_course_response(
    ...
    """
    Проверяет соответствие данных созданного курса данным запроса.

    :param request: Данные, переданные при создании курса.
    :param response: Данные курса из ответа.
    """
    course = response.course

    assert course.title == request.title
    assert course.max_score == request.max_score
    assert course.min_score == request.min_score
    assert course.description == request.description
    assert course.estimated_time == request.estimated_time
    assert course.preview_file.id == request.preview_file_id
    assert course.created_by_user.id == request.created_by_user_id